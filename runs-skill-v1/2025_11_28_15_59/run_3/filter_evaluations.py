...
for metric in parsed_data['model-index'][0]['results'][0]['metrics']:
    ...
    for allowed in allowed_benchmarks:
        if allowed in metric_type:
            is_allowed = True
            # Clean up the metric name and type
            metric['name'] = re.sub(r'\s*\([^)]*\)', '', metric['name']).strip()
            
            # Standardize benchmark names
            if 'mmlu' in metric_type:
                metric['type'] = 'mmlu'
                metric['name'] = 'MMLU'
            elif 'truthfulqa' in metric_type:
                metric['type'] = 'truthfulqa'
                metric['name'] = 'TruthfulQA'
            else:
                metric['type'] = allowed
                # Capitalize the benchmark name
                metric['name'] = allowed.replace('_', ' ').title()
            
            break
    
    if is_allowed and '(random)' not in metric['name'].lower():
        filtered_metrics.append(metric)

# Create the filtered YAML structure
filtered_data = {
    'model-index': [{
        'name': parsed_data['model-index'][0]['name'],
        'results': [{
            'task': {
                'type': 'text-generation'
            },
        ...
        'source': parsed_data['model-index'][0]['source']
    }]
}
...
print(f"Filtered {len(filtered_metrics)}/81 metrics successfully saved to olmo_7b_evaluations.yaml")
